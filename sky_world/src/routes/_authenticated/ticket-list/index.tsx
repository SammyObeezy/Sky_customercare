...
// Updated imports: Link and createFileRoute
...
function TicketList() {
  ...
  const columns: TableColumn[] = [
    { id: 'id', caption: 'Ticket ID', filterable: true, sortable: true, size: 100, align: 'center' },
    {
      ...
      size: 300,
      render: (row) => (
        // UPDATED: Changed from an <a> tag to a type-safe <Link>
        (<Link to="/ticket/$ticketId" params={{ ticketId: row.id.toString() }} className="ticket-link">
          {row.ticketSubject}
        </Link>)
      )
    },
